...
public class AirlineGUI extends JFrame{
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				AirlineGUI app = new AirlineGUI(new RemoteAirlineService());
	...
	public AirlineGUI(AirlineService airline) {
		...
		List<Flight> vuelos = new ArrayList<Flight>(this.airline.listFlights());
		...
		this.listaTickets = new JList<Ticket>();
	...
	public void updateLists() {
		...
		if(this.listaVuelos.getItemCount()>0) {
			Flight vueloSeleccionado = (Flight) this.listaVuelos.getSelectedItem();
			for (Flight f: this.airline.listFlights()) {
				if (f.getFlightNumber().equals(vueloSeleccionado.getFlightNumber())) {
					vueloSeleccionado = f;
				}
			}
			if(vueloSeleccionado != null){
				...
				if(!ticketsList.isEmpty()) {
					this.listaTickets.setListData(new Vector<Ticket>(ticketsList));
					...
					this.listaTickets.setListData(new Vector<Ticket>());
				}
			}
		}
	}
	...
	class SelectionOfTicket implements ListSelectionListener{
		public void valueChanged(ListSelectionEvent e) {
			...
			, "Ticket n�: "+ticket.getTicketNumber(), JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
